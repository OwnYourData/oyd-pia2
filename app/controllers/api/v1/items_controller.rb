			...
			def details
				...
				if !@item.nil?
					...
					if check_permission(repo_identifier, @app, PermType::READ)
						retVal = @item.as_json
						retVal["repo_name"] = @item.repo.name.to_s
						retVal["merkle_hash"] = @item.merkle.root_hash.to_s unless @item.merkle.nil?
						retVal["oyd_transaction"] = @item.merkle.oyd_transaction.to_s unless @item.merkle.nil?
						render json: retVal,
							   status: 200
					else
						render json: { "error": "Permission denied" }, 
			...
			def create_item(repo, user_id, params)
				...
				if input[:_json]
					...
					item_array.each do |item|
						retVal = write_item(repo, item.to_json.to_s)
						return_array << retVal
